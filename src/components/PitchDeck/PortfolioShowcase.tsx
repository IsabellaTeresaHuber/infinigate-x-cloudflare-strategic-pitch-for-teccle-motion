...
import { Shield, Zap, Globe, Cloud, Database, Network } from 'lucide-react';
...
const portfolioItems: PortfolioItem[] = [
  {
    ...
    replacement: "Replaces Legacy VPN & Sophos Firewalls",
  ...
  },
  {
    ...
    replacement: "Superior to Sophos Endpoint/WAF",
  ...
  },
  {
    ...
    replacement: "Replaces Commvault/Pure Storage Silos",
  ...
  },
  {
    ...
    replacement: "Faster than Legacy Infra/CDNs",
      ...
      {portfolioItems.map((item, idx) => (
        ...
        >
          <div className="flex items-start justify-between mb-6">
            ...
            <Badge variant="secondary" className="bg-cloudflare/10 text-cloudflare border-none">
              {item.replacement}
            </Badge>
          </div>
          ...
          <ul className="space-y-3">
            {item.benefits.map((benefit, bIdx) => (
              <li key={bIdx} className="flex items-center gap-2 text-sm font-medium text-foreground/80">
                <div className="w-1.5 h-1.5 rounded-full bg-cloudflare" />
                {benefit}
              </li>
            ))}
          </ul>
